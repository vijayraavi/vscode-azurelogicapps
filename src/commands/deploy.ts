...
export async function deploy(ui: IAzureUserInput, telemetryProperties: TelemetryProperties, tree: AzureTreeDataProvider, outputChannel: vscode.OutputChannel, target?: vscode.Uri | string | IAzureParentNode<FunctionAppTreeItem>, functionAppId?: string | {}): Promise<void> {
    ...
    const functions: IAzureNode<WorkflowCodeViewTreeItem>[] = <IAzureNode<WorkflowCodeViewTreeItem>[]>await functionsNode.getCachedChildren();
    const anonFunctions: IAzureNode<WorkflowCodeViewTreeItem>[] = functions.filter((f: IAzureNode<WorkflowCodeViewTreeItem>) => f.treeItem.config.isHttpTrigger && f.treeItem.config.authLevel === HttpAuthLevel.anonymous);
    ...
    if (functions.find((f: IAzureNode<WorkflowCodeViewTreeItem>) => f.treeItem.config.isHttpTrigger && f.treeItem.config.authLevel !== HttpAuthLevel.anonymous)) {
