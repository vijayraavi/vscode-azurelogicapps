...
export function activate(context: vscode.ExtensionContext): void {
    ...
    const editorManager: WorkflowEditorManager = new WorkflowEditorManager(context.globalState);
    ...
    callWithTelemetryAndErrorHandling('azureFunctions.activate', reporter, outputChannel, async function (this: IActionContext): Promise<void> {
        ...
        actionHandler.registerCommand('azureFunctions.copyFunctionUrl', async (node?: IAzureNode<WorkflowCodeViewTreeItem>) => await copyFunctionUrl(tree, node));
        ...
        actionHandler.registerCommand('azureFunctions.deleteFunction', async (node?: IAzureNode) => await deleteNode(tree, WorkflowCodeViewTreeItem.contextValue, node));
        ...
        // Explorer
        actionHandler.registerCommand('azureLogicApps.deploy', async function (this: IActionContext, deployPath: vscode.Uri | string, functionAppId?: string): Promise<void> { await deploy(ui, this.properties, tree, outputChannel, deployPath, functionAppId); });
    });
}
